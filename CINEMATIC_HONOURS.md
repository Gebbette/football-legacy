...
Presentation is shared by Original, Express, Quick Story and Multiplayer through `honours-cinema.js` and `honours-cinema.css`. All seven original SVG trophies are now embedded directly in the renderer, so blocked or missing separate image files cannot produce broken-image icons. Editable SVG sources remain in `assets/honours/`. Renderer cache version is 85. No runtime dependency or external font was added.
